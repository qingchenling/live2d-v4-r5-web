...
export class LAppDelegate {
  ...
  public initialize(): boolean {
    ...
    document.addEventListener("mousemove", function (e) {
      ...
      let posX:number = e.clientX, posY: number = e.clientY;

      if(posX > w/2) {
        posX = (posX - w/2)/(document.documentElement.clientWidth - w/2)*w/2 + w/2;
      }
      if(document.documentElement.clientHeight-posY > h/2) {
        posY = posY/(document.documentElement.clientHeight - h/2)*h/2;
      }
      else {
        posY -= document.documentElement.clientHeight - h;
      }
